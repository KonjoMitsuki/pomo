# Discord ポモドーロタイマー Bot

Discord のボイスチャンネルで使えるポモドーロタイマー Bot です。  
作業と休憩を自動で繰り返し、参加者ごとの作業時間とセッション数を記録します。

## 主な機能

- ポモドーロ自動ループ（小休憩 / 長休憩）
- カウントアップタイマー（無限ループ・手動停止）
- 参加・退出ボタン（🙋 / 👋）
- ホスト退出時のホスト自動移行
- VC退出メンバーの自動除外
- 一時停止 / 再開 / 終了ボタン
- 統計保存（SQLite）
- 通知音のミュート切り替え（`!mute`）
- VOICEVOX による動的音声アナウンス

## ディレクトリ構成

```text
pomo/
├─ README.md
├─ start_pomo.sh
├─ assets/
│  ├─ pomo.db
│  └─ voicevox_temp.wav  # 実行時に生成される一時ファイル
├─ documents/
│  ├─ manual.md
│  ├─ specification.md
│  └─ pomo_refactor_design.md
└─ src/
   ├─ audio.py
   ├─ cog.py
   ├─ runner.py
   ├─ session.py
   ├─ storage.py
   ├─ timer.py
   └─ views.py
```

## 依存関係

```bash
pip install discord.py aiosqlite aiohttp
```

音声通知のため FFmpeg も必要です。
VOICEVOX エンジンは `http://127.0.0.1:50021` で起動しておいてください。

```bash
# Ubuntu / Debian
sudo apt install ffmpeg
```

## セットアップ

1. Bot トークンを環境変数に設定

```bash
export DISCORD_BOT_TOKEN='your_token_here'
```

2. Bot を起動

```bash
python src/timer.py
```

または:

```bash
./start_pomo.sh
```

起動成功時はログインメッセージが表示されます。

## コマンド

### `!pomo [timer_name]` / `!np [timer_name]`

`!pomo`: 名前付きタイマーを指定してポモドーロを開始します。
`!np`: カウントアップタイマーを開始します。手動で終了するまで経過時間を記録します。
タイマー名を省略すると `original` を使用します。

例:

```bash
!pomo
!pomo work
!np work
```

タイマー設定は `!pconfig` で作成・上書きできます。指定がない場合はデフォルト設定（`25 5 15 4`）が使われます。

### `!timer`

現在の稼働中タイマーの設定、進捗、参加者ごとの今回作業時間を表示します。Embed に `タイマー名` が表示されます。

### `!add @user` / `!remove @user` / `!plist`

参加対象の追加・削除・および `!plist` であなたのタイマー設定一覧（サーバー単位）を表示します。

`!plist` は `!pl` の短縮も使用できます。既存の `!list` は `!plist` に置き換わりました。

### `!stats [timer_name]` / `!reset`

`!stats`（引数なし）: 全タイマーの累計作業時間一覧を表示します。

`!stats <timer_name>`: 指定タイマーの詳細統計（累計作業時間・完了セッション）を表示します。

`!reset` はあなたの統計をリセットします。

### `!mute`

通知音のミュートを切り替えます（再実行で解除）。

### `!test`

VC で VOICEVOX 音声再生テストを行います。

### `!pconfig` / `!pdel` / `!help`

`!pconfig <name> [work] [short] [long] [interval]` — 名前付きタイマーの作成・上書き（デフォルト: `25 5 15 4`）。

`!pdel <name>` — 指定タイマーを削除（記録が残っている場合は削除不可）。

`!help` — コマンド一覧を表示します。

## ボタン操作

- `⏸️ 一時停止`: カウント停止
- `▶️ 再開`: カウント再開
- `⏹️ 終了`: タイマー終了と Bot 退出
- `🙋 参加`: タイマー対象に参加
- `👋 退出`: タイマー対象から退出

## 動作のポイント

- ホストまたは参加者が VC に残っている限り継続
- 全員が VC から抜けると自動終了
- 長休憩は「完了セッション数 % 長休憩頻度 == 0」で発生
- 作業時間は VC 内の対象者のみ加算

## データベース

`assets/pomo.db` では、記録を4つのテーブルに分けて保存します。

| テーブル名        | 役割                                             |
| :---------------- | :----------------------------------------------- |
| `users`           | Discordユーザーの基本情報を保存                  |
| `timers`          | タイマー設定を保存                               |
| `sessions`        | ポモドーロ1回ごとの開始・終了記録を保存          |
| `session_members` | セッションごとの参加者別の作業時間と完了数を保存 |

この構成により、単なる合計値だけでなく「いつ」「どのタイマーで」「誰がどれだけ作業したか」を追跡できるようになりました。

## トラブルシューティング

### 音が鳴らない

```bash
ffmpeg -version
curl -s http://127.0.0.1:50021/version
```

その後、`!test` で確認してください。

### 起動しない

```bash
echo $DISCORD_BOT_TOKEN
```

空なら環境変数が未設定です。

## セキュリティ注意

- トークンはコードに直接書かない
- `.env` を使う場合は `.gitignore` に追加

```bash
echo ".env" >> .gitignore
```

---

Version: 2026-04
