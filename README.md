...
- `src/timer.py` - メインプログラム
- `assets/ding.mp3` - 通知音
- `assets/pomo.db` - 統計データベース（自動生成）
- `documents/` - 詳細仕様や設計書
...
6. 休憩終了時に音声通知
7. 次のセッションを自動開始
8. 4回目（または指定回数）のセッション後は長休憩
9. ボイスチャンネルから退出するまで無限継続
...
コマンド実行後、メッセージに表示されるボタンで操作できます:

- **⏸️ 一時停止** - タイマーを一時停止（カウントダウンを停止）
- **▶️ 再開** - 一時停止したタイマーを再開
- **⏹️ 終了** - タイマーを終了してボイスチャンネルから退出

**注意:** ボタン操作はコマンドを実行したユーザーのみ可能です。
...
2. **ボイスチャンネルから退出** - 自動的に終了

どちらの場合も、累計セッション数が表示されます。
...
2. ボットがボイスチャンネルに接続
3. 作業タイマー開始（1分ごとにメッセージ更新）
4. 作業完了時:
   - データベースに記録
   - 音声通知を再生（assets/ding.mp3）
5. 自動的に休憩タイマー開始
   - 4の倍数セッション後 → 長休憩
   - それ以外 → 小休憩
6. 休憩完了時:
   - 音声通知を再生
7. 2秒待機後、次のセッションを自動開始
8. ユーザーがボイスチャンネルから退出するまで継続

## データベース構造

SQLiteデータベース（`assets/pomo.db`）にユーザーごとの統計を保存します。
...
**5. 音量設定を調整**
音が小さい場合は、`src/timer.py`から参照される`src/runner.py`の音量設定を変更できます:

```python
# src/runner.py の作業終了・休憩終了の再生箇所
...
コマンドを実行したユーザー以外はボタンを操作できません。これは仕様です。
...
`src/timer.py`から参照される`src/runner.py`の音量設定を変更できます:
...
`assets/ding.mp3`を別の音声ファイルに置き換えるか、`src/timer.py`の`SOUND_FILE`変数を変更:
